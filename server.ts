...
// Endpoint to get Plesk Windows deployment details and configuration files
app.get('/api/plesk/info', (req, res) => {
  try {
    const webConfigPath = path.join(process.cwd(), 'web.config');
    const serverJsPath = path.join(process.cwd(), 'server.js');
    const readmePath = path.join(process.cwd(), 'README_PLESK_WINDOWS.md');

    const webConfigContent = fs.existsSync(webConfigPath) ? fs.readFileSync(webConfigPath, 'utf-8') : '';
    const serverJsContent = fs.existsSync(serverJsPath) ? fs.readFileSync(serverJsPath, 'utf-8') : '';
    ...
    res.json({
      ...
      environment: {
        ...
        iisNodeCompatible: true,
        port: PORT,
      },
      files: {
        webConfig: webConfigContent,
      ...
      pleskSettings: {
        ...
        applicationStartupFile: 'server.js',
        nodeVersionsRecommended: ['20.x', '22.x'],
        packageManager: 'npm',
      },
    });
  } catch (err: any) {
...
app.get('/api/plesk/download-package', async (req, res) => {
  try {
    ...
    for (const relPath of PLESK_WINDOWS_DEPLOY_FILES) {
    ...
    // 2. Add all files in /src recursively
