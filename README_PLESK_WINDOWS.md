...
2. **การรันคำสั่ง `build` บน Plesk (รองรับ Node 18.20.6 เรียบร้อยแล้ว 100%):**
   - ระบบได้ใส่สคริปต์แก้ไข `scripts/patch-node18.cjs` และ Polyfill ป้องกันข้อผิดพลาด `styleText` ไว้ให้ในแพ็กเกจ
   - ทำให้ท่านสามารถเลือกคำสั่ง **`run build`** (หรือ **`build:plesk`**) ในหน้า Plesk Control Panel ได้โดยตรง การ Build จะทำงานสำเร็จผ่านฉลุย
   - หรือหากไม่ต้องการรัน Build ใหม่ โฟลเดอร์ `dist/` ในแพ็กเกจ ZIP ก็ได้รับการคอมไพล์เตรียมไว้ล่วงหน้าแล้วเช่นกัน
