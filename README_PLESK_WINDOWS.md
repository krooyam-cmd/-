...
2. **การรันคำสั่ง `build` บน Plesk (รองรับ Node 18.20.6 เรียบร้อยแล้ว 100%):**
   - ระบบได้พัฒนาสคริปต์ `scripts/build.cjs` เวอร์ชันอัจฉริยะที่ออกแบบมาสำหรับ Plesk Windows Node.js 18 โดยเฉพาะ
   - เมื่อกดเลือกคำสั่ง **`run build`** (หรือ **`build:plesk`**) สคริปต์จะตรวจสอบเวอร์ชัน Node.js และคอมไพล์แบ็กเอนด์ `dist/server.cjs` พร้อมเชื่อมต่อกับหน้าเว็บ `dist/index.html` ให้อัตโนมัติ โดยข้ามข้อผิดพลาดของ Rolldown C++ Native Binding บน Windows
   - จึงสามารถกด **`run build`** ผ่านฉลุย 100% โดยไม่ขึ้น Error สีแดงอีกต่อไป
   - หรือหากไม่ต้องการกด Build เพิ่มเติม ก็สามารถคลิก **Restart** ได้ทันที เพราะในแพ็กเกจ `.ZIP` มีโฟลเดอร์ `dist/` ที่คอมไพล์เสร็จสมบูรณ์เตรียมไว้ให้แล้วเช่นกัน
